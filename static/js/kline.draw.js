...
MainDataSource.prototype.getDatas = function(){
    ...
    if (h > 0) {
        ...
        for (f = 0; f < b; f++) {
            ...
            for (; f < b; f++, this._appendedCount++) {
                j = c[f];
                var last_index=this._dataItems.length-1;
                if(parseInt(this._dataItems[last_index].date/1000/60)==parseInt(j[0]/1000/60)){
                    this._dataItems[last_index]={
                        date: j[0],
                        open: j[1],
                        high: j[2],
                        low: j[3],
...
MainInfoPlotter.prototype.Draw = function(c) {
    ...
    var ms = format_time(g.getSeconds());
    var D = B.getLanguage();
    if (D == "zh-cn") {
        if (!Plotter.drawString(c, "时间: " + k + "-" + A + "-" + z + "  " + f + ":" + l + ":" + ms, a)) {
            return
        }
            ...
            if (GLOBAL_VAR.G_HTTP_REQUEST) {
                ...
                try {
                    if (!GLOBAL_VAR.chartMgr.updateData("frame0.k0", GLOBAL_VAR.KLineData)) {
                        //GLOBAL_VAR.requestParam = setHttpRequestParam(GLOBAL_VAR.market_from, GLOBAL_VAR.time_type, GLOBAL_VAR.limit, null);
                        //推送点下次请求
                        //GLOBAL_VAR.TimeOutId = setTimeout(RequestData, 2*1000);
                        return
                    }
...
function websockets(showLoading){
    ...
    socket.onmessage = function(e){
        ...
        //    d[i]=parseInt(d[i]);
        GLOBAL_VAR.market_from_name = "凯瑞投资";
                var chart = ChartManager.getInstance().getChart();
                chart._contract_unit = "BTC";
                chart._money_type = "CNY";
                chart._usd_cny_rate = 6.83;
                chart.setTitle();
                //kline.setMarketShow(GLOBAL_VAR.market_from_name, chart._contract_unit, chart._money_type, json.datas.url);
                //kline.setTopTickers(json.datas.topTickers);
                //GLOBAL_VAR.KLineData = eval(json.datas.data);
                ...
                if (!GLOBAL_VAR.chartMgr.updateData("frame0.k0", GLOBAL_VAR.KLineData)) {
                        ...
                        //alert(GLOBAL_VAR.KLineData[0])
                        return
                    }
                    clear_refresh_counter()
                //自循环请求点，测试用，页面启动后会每隔8秒请求一次后台点数据（一个点）
                //GLOBAL_VAR.TimeOutId = setTimeout(TwoSecondThread, 60 * 60 * 3 * 1000);
                $("#chart_loading").removeClass("activated");
                ChartManager.getInstance().redraw("All", false);
                GLOBAL_VAR.G_HTTP_REQUEST = null;
    }
